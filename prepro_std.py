    ...
    def build_data_sequence(data, dump_path, max_seq_len=MAX_SEQ_LEN, tokenizer=None, label_mapper=None):
        with open(dump_path, 'w', encoding='utf-8') as writer:
            for idx, sample in enumerate(data):
                ...
                lenn = sample['lenn']
                drug = sample['drug']
                tokenss = sample['token']
                sentence_input = sample['sentence_input']
                start_sentence = sample['start_sentence']
                len_sentence = sample['len_sentence']
                ade = sample['ade']
                ...
                features = {'uid': ids, 'label': label, 'token_id': input_ids, 'type_id': type_ids, 'tokens': tokenss, 'start': start, 'section': section, 'lenn': lenn, 'drug': drug, 'sentence_input': sentence_input, 'start_sentence': start_sentence, 'len_sentence': len_sentence, 'ade': ade, 'modifier': modifier}
                writer.write('{}\n'.format(json.dumps(features)))
...
def load_data(file_path, data_format, task_type, label_dict=None):
    ...
    for line in open(file_path, encoding="utf-8"):
        ...
        if data_format == DataFormat.PremiseOnly:
            ...
            row = {"uid": fields[0], "label": eval(fields[1]),  "premise": eval(fields[2]), "start": eval(fields[3]), "section": eval(fields[4]), "lenn": eval(fields[5]), "drug": eval(fields[6]), "sentence_input": eval(fields[7]), "start_sentence": eval(fields[8]), "len_sentence": eval(fields[9]), "token": eval(fields[10]), "ade": eval(fields[11]), "modifier": eval(fields[12])}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Preprocessing ADE dataset.')
    parser.add_argument('--model', type=str, default='bert-base-uncased',
                        help='bert-base-uncased/bert-large-uncased/reberta-large')
    ...
    parser.add_argument('--roberta_path', type=str, default="pretrained_models/roberta")
    
    parser.add_argument('--model-type', type=str, default='scibert',
                       help='scibert/bert/biobert/roberta/bluebert')
...
def main(args):
    ...
    if args.model_type == 'scibert':
        encoder_model = EncoderModelType.SCIBERT
        
    elif args.model_type == 'bert':
        encoder_model = EncoderModelType.BERT
        
    elif args.model_type == 'biobert':
        encoder_model = EncoderModelType.BIOBERT
        
    elif args.model_type == 'bluebert':
        encoder_model = EncoderModelType.BLUEBERT
        
    elif args.model_type == 'roberta':
        encoder_model = EncoderModelType.ROBERTA
		
    elif args.model_type == 'pubmedbert':
        encoder_model = EncoderModelType.PUBMEDBERT
    ...
    elif encoder_model == EncoderModelType.BIOBERT:
        print ('biobert')	
        tokenizer = BertTokenizer(
            'pretrained_models/biobert_vocab/vocab.txt', do_lower_case=do_lower_case)
    elif encoder_model == EncoderModelType.SCIBERT:
        print ('scibert')	
        tokenizer = BertTokenizer(
            'pretrained_models/scibert_vocab/vocab.txt', do_lower_case=do_lower_case)
    elif encoder_model == EncoderModelType.BLUEBERT:
        print ('bluebert')	
        tokenizer = BertTokenizer(
            'pretrained_models/bluebert_vocab/vocab.txt', do_lower_case=do_lower_case)
    elif encoder_model == EncoderModelType.PUBMEDBERT:
        print ('pubmedbert')	
        tokenizer = BertTokenizer(
            'pretrained_models/pubmedbert_vocab_a/vocab.txt', do_lower_case=do_lower_case)
    else:
